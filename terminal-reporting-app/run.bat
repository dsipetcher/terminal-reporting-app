...
echo.
echo  ========================================
echo   ИЛС — угольно-нефтяной терминал
echo  ========================================
echo.
...
echo.
echo  Запуск: http://localhost:3001
...
echo [1/5] Загрузка portable Node.js - интернет, один раз...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\ensure-node.ps1"
if exist "%~dp0runtime\node\node.exe" (
    set "NODE_EXE=%~dp0runtime\node\node.exe"
    goto :eof
)
echo [ОШИБКА] Node.js недоступен. Запустите prepare-portable.bat
exit /b 1
...
echo [3/5] Создание базы и демо-данных...
call :run_db_setup
if errorlevel 1 exit /b 1
goto :eof
